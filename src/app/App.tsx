...
export default function App() {
  ...
  /**
   * Launch a new terminal session with the given command.
   * Used by the command palette to start Claude Code, Codex, Open Code, etc.
   */
  const launchSession = useCallback(
    async (command: string, _label: string) => {
      ...
      let agentType: "claude" | "codex" | "opencode" | undefined;
      let agentTitle = "shell";
      if (command === "claude") {
        agentType = "claude";
  ...
  const handleNewSpace = useCallback(
    (dir?: string, name?: string) => {
      ...
      const root = dir ?? activeCwd ?? home ?? null;
      const meta = create({
        name: name ?? `工作区 ${spaces.length + 1}`,
        root,
        env: workspaceEnv,
      });
  ...
  const handleNewSpaceWithDir = useCallback(async () => {
    const dir = await open({ directory: true, title: "选择工作区目录" });
    if (dir) {
      const name = dir.split(/[\\/]/).filter(Boolean).pop() ?? dir;
      handleNewSpace(dir, name);
    }
  }, [handleNewSpace]);
  ...
  const spaceSwitcher = (
    <SpaceSwitcher
      ...
      tabs={tabs}
      onNewSpace={() => void handleNewSpaceWithDir()}
    ...
    () =>
      commandPaletteOpen
        ? createCommandItems({
            ...
            newSpace: () => void handleNewSpaceWithDir(),
